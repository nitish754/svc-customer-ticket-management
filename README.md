...
## Queue UI (Bull Board)
Open `http://localhost:3000/admin/queues` to visualize jobs, retries, and failures.
...
### Force failures for demo (optional)
Set a fail rate between `0` and `1`:
```bash
CRM_FAIL_RATE=1 npm run start:dev
```
- `1` = always fail (good for retry demo)
- `0.5` = fail 50% of the time
- Default is `0.35`
